# ========================================
# Flight Tracker - With Delay Calculations
# ========================================
...
class TimedFlight : Flight {
	...
	[void]PrintWithDelta() {
		...
		$color = if ($delta.TotalMinutes -lt 0) { "Green" } else { "Red" }
...
Write-Host "                  FLIGHT TRACKER - WITH DELAY TRACKING" -ForegroundColor Yellow
Write-Host ("="*80) -ForegroundColor Green
Write-Host ""

# Example flights with delays
$timedFlights = @(
	# ANA 107 - 10 minutes early
	[TimedFlight]::new(
		"ANA 107","SFO → HND",
		(DT 2026 7 2 1 40),"PDT",
		(DT 2026 7 3 4 35),  # scheduled
		(DT 2026 7 3 4 25),  # actual (10 min early)
		"JST",
		([timespan]::FromMinutes(10*60+45)),
		"34A, 34C","✓ Arrived"
	),

	# ANA 855 - 15 minutes late
	[TimedFlight]::new(
		"ANA 855","HND → CGK",
		(DT 2026 7 3 10 15),"JST",
		(DT 2026 7 3 16 0),   # scheduled
		(DT 2026 7 3 16 15),  # actual (15 min late)
		"WIB",
	...
	# ANA 108 - 5 minutes early
	[TimedFlight]::new(
		"ANA 108","HND → SFO",
		(DT 2026 7 20 22 55),"JST",
		(DT 2026 7 20 16 55),  # scheduled
		(DT 2026 7 20 16 50),  # actual (5 min early)
		"PDT",
